...
interface GetAllRecentlyAddedMealRequest{
    page: number
    limit: number
    meal_name?: string
...
export async function getAllRecentlyAddedMeals(request: GetAllRecentlyAddedMealRequest): Promise<ResponseProps<MealProps[]>>{
    let url = `/api/meal/recent?page=${request.page}&limit=${request.limit}`
